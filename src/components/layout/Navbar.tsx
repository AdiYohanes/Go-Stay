...
import { Menu, User, LogOut, Calendar, Globe, Search as SearchIcon } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function Navbar() {
  ...
  return (
    <nav className="border-b bg-background sticky top-0 z-50 h-[80px]">
      <div className="container h-full mx-auto px-4 md:px-10 flex items-center justify-between">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-1 text-primary">
           ...
           <span className="font-bold text-xl hidden md:block text-primary">go-stay</span>
        </Link>
        ...
        <div className="md:hidden flex-1 flex justify-center px-4">
           <div className="rounded-full shadow-sm border p-2 flex items-center gap-2 w-full max-w-[300px] bg-background">
        ...
        <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-full border border-border p-1 pl-3 hover:shadow-md transition-shadow ml-1">
                  <Menu className="h-4 w-4" />
                  <Avatar className="h-8 w-8">
                    <AvatarImage src={user?.user_metadata?.avatar_url} alt={user?.email || ''} />
                    <AvatarFallback className="bg-muted-foreground/30 text-xs">
                        {isLoading ? '' : user?.email?.charAt(0).toUpperCase() || <User className="h-4 w-4 text-foreground" />}
                    </AvatarFallback>
                  </Avatar>
                </button>
              </DropdownMenuTrigger>
                    ...
                    <>
                        <DropdownMenuLabel className="font-normal p-4 pb-2">
                             <div className="flex flex-col space-y-1">
                                <p className="text-sm font-medium leading-none">{user.user_metadata?.full_name || 'Guest'}</p>
                        ...
                        <DropdownMenuItem className="py-2.5 font-medium">Wishlists</DropdownMenuItem>
                        <DropdownMenuSeparator />
                     ...
                     <>
                        <DropdownMenuItem className="font-semibold py-2.5 text-base" asChild>
