...
export function Navbar() {
  ...
  return (
    <nav className={cn(
      ...
      isHomePage 
        ? (isScrolled ? "fixed bg-background/80 backdrop-blur-md shadow-sm" : "fixed bg-transparent")
                ...
                <button className={cn(
                  ...
                  <Avatar className="h-8 w-8">
                    ...
                    <AvatarFallback className={cn(
                      "text-xs",
                      isHomePage && !isScrolled ? "bg-white/30 text-white" : "bg-muted-foreground/30"
                    )}>
                        {isLoading ? '' : user?.email?.charAt(0).toUpperCase() || <User className="h-4 w-4" />}
                    </AvatarFallback>
                  </Avatar>
                </button>
              </DropdownMenuTrigger>
